...
static int __init print_board_info (void)
{
	...
	if (cpu_is_imx6ul())
        ...
        printk ("\tSOM Version     : TRUX-iMX6UL-D01\n");
...
static const struct of_dev_auxdata imx6_truxd01_auxdata_lookup[] __initdata = { 
	...
	if (parent == NULL)
		pr_warn("failed to initialize soc device\n");
      	...
      	of_platform_populate(NULL, of_default_bus_match_table,
                              imx6_truxd01_auxdata_lookup, NULL);
	imx6ul_enet_init();
	imx_anatop_init();
	imx6ul_pm_init();
	print_board_info();
